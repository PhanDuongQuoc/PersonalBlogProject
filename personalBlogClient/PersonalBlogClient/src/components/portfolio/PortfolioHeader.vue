    ...
    <q-dialog v-model="mobileMenuOpen" position="left" full-height persistent>
      <aside class="mobile-drawer" @click.stop @mousedown.stop @touchstart.stop>
        ...
        <nav class="mobile-navigation">
          <a v-for="item in navigation" :key="item.target" :href="item.target" :class="{ active: activeTarget === item.target }" @click.stop.prevent="navigate(item.target)">
        ...
        <div class="mobile-drawer-footer">
          ...
          <a class="drawer-resume" href="#contact" @click.stop.prevent="navigate('#contact')">{{ text.resume }} <q-icon name="arrow_forward" /></a>
        </div>
      </aside>
    </q-dialog>
  </q-header>
